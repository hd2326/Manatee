...
#
#     def __call__(self, X, X_rec, Z, Z_rec, mu, logvar):
#         """ Compute loss."""
...
#         loss = torch.mean((1 - self.alpha * loss_1) + self.alpha * loss_2)
#         return loss
#
...
class KFoldTorch:
    ...
    def train_kfold(self, blank_model, model_params, dataset, batch_size, drop_last_batch=True):
        ...
        for i, (train_idx, test_idx) in enumerate(kfold.split(dataset, dataset.targets)):
            ...
            if 'valid_loss' in epoch_hist.keys():
                ...
                if best_val_loss > np.min(epoch_hist['valid_loss']):
                    ...
                    for layer_name in best_model: best_model[layer_name] = best_model[layer_name].cpu()
            # Save if save all
            if self.save_all:
                full_path = self.path_dir+self.model_prefix+'_fold_'+str(i)+'.pt'
        ...
        if self.save_best and not self.save_all:
            ...
            torch.save(best_model, path_best)
